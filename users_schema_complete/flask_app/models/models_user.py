...
class User:
    ...
    def get_one(cls, data): #no need for data bc no id, it's all of them
        query="""
            ...
            """
        user_id= connectToMySQL(cls.db).query_db(query, data) #the controller specifies which ID
        return cls(user_id[0])

    @classmethod
    def get_all(cls): #no need for data bc no id, it's all of them
        query="SELECT * FROM users;"
        results= connectToMySQL(cls.db).query_db(query) #return results would look weird like a long dictionary to parse through, so do this:
        users=[]
        for user in results:
            #makes an object
